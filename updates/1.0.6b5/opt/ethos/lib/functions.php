...
function get_stats(){
	...
	$send['connected_displays'] = trim(`export DISPLAY=:0 && /usr/bin/xrandr | grep " connected" | cut -f3 -d" " | cut -f1 -d"+" | xargs`);
	$send['resolution'] = trim(`export DISPLAY=:0 && /usr/bin/xrandr | grep current | cut -d" " -f 8,10 | cut -d"," -f1 | xargs`);
