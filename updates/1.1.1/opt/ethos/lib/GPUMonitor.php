...
class GPUMonitor
{
    ...
    protected function scanNewMinerOutputData()
    {
        ...
        while (! feof($this->fh_miner)) {
            ...
            if (preg_match('/m  .*\d\d:\d\d:\d\d.*ethminer.*  Mining on PoWhash .*#[a-z0-9]+.* : (\d+.\d+)MH\/s .*A\d+\+\d+:R\d+\+\d+:F\d+\.*/i', $data, $match)) {
                $seconds = "0";
		$millisec = EthosConf::get("farmrecheck");;
		$multiplier = ".$millisec";
		$hashes = $match[1] * 1000000 * $multiplier ;
                // $millisec = "200";

                $this->addMinerLogData($hashes, $seconds, $millisec);
                //$this->addMinerLogData($hashes);
            }
        }
    }

    protected function addMinerLogData($hashes, $seconds, $millisec)
    //protected function addMinerLogData($hashes)
    {
